...
#        return amr
    ...
    def combine_multiple(self, hgs):
        ...
        multi_str += ')'
        return Hgraph.from_string(multi_str)
    ...
    def find_nt_edge(self, label, index):
        #print >>sys.stderr, '====In find_nt_edge=========='
        #print >>sys.stderr, self
        #print >>sys.stderr, self.triples()
        #print >>sys.stderr, label
        #print >>sys.stderr, index
        for p,r,c in self.triples():
            if type(r) is NonterminalLabel:
                #print >>sys.stderr, '--------NonterminalLabel----'
                #print >>sys.stderr, r.label
                #print >>sys.stderr, r.index
                if r.label == label and r.index == index:
                    #print >>sys.stderr, 'exit 1'
                    #sys.exit(0)
                    return p,r,c
        #print >>sys.stderr, 'exit 2'
        #sys.exit(0)
